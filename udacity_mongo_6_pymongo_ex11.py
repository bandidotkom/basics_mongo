    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for key in FIELDS.keys():
                ...
                if val=="NULL":
                    val = None
                if key=="rdf-schema#label" and len(val.split(" "))>1:
                    val = val.split(" ")[0]
                if key=="name" and (val==None or re.match(r'\W', val)):
                    val = obj["label"]
                if key=="synonym" and val!=None:
                    val = parse_array(val)
                if key=='family_label' or key== 'class_label' or key=='phylum_label' or key== 'order_label' or key=='kingdom_label' or key=='genus_label':
                    classification[FIELDS[key]] = val
                else:
                    obj[FIELDS[key]] = val
