...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for tr in soup.find_all("tr", { "class" : "dataTDRight" }):
            ...
            if dataRow[1]!="TOTAL":
                info2 = {}
                info2["courier"] = info["courier"]
                info2["airport"] = info["airport"]
                info2["year"] = int(dataRow[0])
                info2["month"] = int(dataRow[1])
                flights = {}
                flights["domestic"] = int(dataRow[2].replace(",", ""))
                flights["international"] = int(dataRow[3].replace(",", ""))
                info2["flights"] = flights
                data.append(info2)
    return data
